...
class OboInfo:
  ...
  def __init__(self):
    ...
    self.children = {}
    self.ont_type = "ECO"
    self.the_ont_root_id = OboInfo.eco_root_id # ECO:0000000
    self.the_ont_annot_root_id = OboInfo.annot_root_id # ECO:9999998
    self.the_ont_rest_ow_id = OboInfo.rest_ow_id # ECO:9999999
  ...
  def set_ont_type(self, otype):
    ...
    if otype=="RZ":
      self.the_ont_root_id = OboInfo.rz_root_id # RZ:0000005
      self.the_ont_annot_root_id = OboInfo.rz_annot_root_id # RZ:9999998
      self.the_ont_rest_ow_id = OboInfo.rz_rest_ow_id # RZ:0000000
    elif otype=="GO":
  ...
  def get_no_match_label(self):
    return self.the_ont_rest_ow_id #OboInfo.rest_ow_id
    
  def get_ont_root_label(self):
    return self.the_ont_root_id #OboInfo.eco_root_id
    
  def get_ont_root_pos(self):
    return self.in_order.index(self.the_ont_root_id) #(OboInfo.eco_root_id)
  ...
  def build_ont_tree(self, fname, obo_type):
    ...
    the_term.children.append(self.the_ont_root_id) #(OboInfo.eco_root_id)
    ...
    with open(fname, "r") as infile:
      ...
      for l in infile:
        ...
        if in_term:
          ...
          if field_name in OboInfo.fields:
            # Handle fields we care about
            val = line[cpos+1:].strip()
            if field_name == "id":
              if val not in OboInfo.remove_ids:
                the_term.id = val
                #print "ONT ID:", val
          ...
          elif field_name in OboInfo.ignore:
            i=1 # ignore
          else:
    ...
    #the_ont = self.ont_terms[OboInfo.eco_root_id]
    #the_ont.parents.append(OboInfo.annot_root_id)
    ...
    for ont_id in self.in_order:
      ...
      if the_ont.id == self.the_ont_annot_root_id: #OboInfo.annot_root_id or the_ont.id==OboInfo.rz_annot_root_id:
